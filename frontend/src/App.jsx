...
export default function App() {
  ...
  const [imageUrl, setImageUrl] = useState("http://localhost:5000/images/iron-sword.png");
  ...
  const submitProduct = async () => {
    try {
      ...
      // 1) save metadata to backend (off-chain file hosting)
      const payload = {
        name,
        description,
        image: imageUrl,
      ...
      const url = out.url; // contoh: http://localhost:5000/metadata/1.json

      // 2) setURI on-chain (MetaMask confirm)
      await regContract.methods.setURI(tokenId, url).send({ from: account });

      // 3) mint on-chain (MetaMask confirm)
      await tokenContract.methods.mint(account, tokenId, amount, "0x").send({ from: account });

      alert("Submit sukses ✅ (metadata saved + setURI + mint)");
      await loadToken();
    } catch (e) {
    ...
    <div className="page">
      <header className="top">
        ...
        <div className="actions">
          ...
          <button className="btn" onClick={loadToken} disabled={!tokenContract || account === "-"}>Load Token</button>
        </div>
      </header>
